...
class Driver():
    ...
    def scale_data(self):
        ...
        self.df[self.predicting_col] = self.scaler.fit_transform(self.df[self.predicting_col].values.reshape(-1,1))
    ...
    def gru_model(self):
        self.model = GRU(input_dim=self.input_dim, hidden_dim=self.hidden_dim, output_dim=self.output_dim, num_layers=self.num_layers)
        self.criterion = torch.nn.MSELoss(reduction='mean')
        self.optimiser = torch.optim.Adam(self.model.parameters(), lr=0.01)
