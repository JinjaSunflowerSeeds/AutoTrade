...
def evening_star_doji(candles_df,
                      ...
                      is_reversed=False,
                      target=None):
    esd = __create_object('EveningStarDoji', target)
    return esd.has_pattern(candles_df, ohlc, is_reversed)

def evening_star(candles_df,
               ohlc=__default_ohlc,
               is_reversed=False,
               target=None):
    es = __create_object('EveningStar', target)
    return es.has_pattern(candles_df, ohlc, is_reversed)
